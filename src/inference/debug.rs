...
#[must_use]
pub fn type_for_slot_at(index: usize, state: &InferenceState) -> Option<TypeVariable> {
...
impl HasInferences for InferenceState {
    /// Returns the inference set for `tv`.
    fn inferences_for(&mut self, tv: TypeVariable) -> InferenceSet {
    ...
    /// Gets all of the type variables that the inference state knows about.
    fn all_type_vars(&mut self) -> Vec<TypeVariable> {
        self.variables()
    }
}
...
/// `state`.
///
/// To print the entire state see [`print_state`], and to print the inference
