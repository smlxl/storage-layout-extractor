...
impl InferenceRule for DynamicArrayWriteRule {
    ...
    fn infer(&self, value: &BoxedVal, state: &mut InferenceState) -> Result<()> {
        let SVD::StorageWrite { key: b, value: g } = &value.data else {
            return Ok(());
        };
        let SVD::StorageSlot { key: c } = &b.data else {
            return Ok(());
        };
        let SVD::DynamicArrayAccess { slot: d, index: f } = &c.data else {
            return Ok(());
        };
        let SVD::StorageSlot { .. } = &d.data else {
            return Ok(());
        };
