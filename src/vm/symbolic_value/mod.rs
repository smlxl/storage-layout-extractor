...
pub struct SymbolicValue {
    ...
    /// Whether the `data` at this level was constructed synthetically.
    #[derivative(PartialEq = "ignore", Hash = "ignore")]
    pub synthetic: bool,
...
impl SymbolicValue {
    /// Constructs a new, non-synthetic, `SymbolicValue` representing the
    /// operation performed at `instruction_pointer` on the symbolic `data`.
    ///
    ...
    pub fn new(instruction_pointer: u32, data: SymbolicValueData) -> Box<Self> {
        Self::new_raw(instruction_pointer, data, false)
    }

    /// Constructs a new, synthetic, `SymbolicValue` representing the operation
    /// performed at `instruction_pointer` on the symbolic `data`.
    ///
    /// It returns [`Box<Self>`] as in the vast majority of cases this type is
    /// used in a recursive data type and hence indirection is needed.
    pub fn new_synthetic(instruction_pointer: u32, data: SymbolicValueData) -> Box<Self> {
        Self::new_raw(instruction_pointer, data, true)
    }

    /// Constructs a new, `SymbolicValue` representing the operation performed
    /// at `instruction_pointer` on the symbolic `data`.
    ///
    /// It returns [`Box<Self>`] as in the vast majority of cases this type is
    /// used in a recursive data type and hence indirection is needed.
    fn new_raw(instruction_pointer: u32, data: SymbolicValueData, synthetic: bool) -> Box<Self> {
        Box::new(Self {
            instruction_pointer,
            synthetic,
            data,
...
mod test {
    ...
    fn equality_ignores_instruction_pointer() {
        ...
        let value_1 = SymbolicValue::new(0, data.clone());
        let value_2 = SymbolicValue::new(0, data);

        assert_eq!(value_1, value_2);
    }

    #[test]
    fn equality_ignores_synthetic_flag() {
        let id = Uuid::new_v4();
        let data = SymbolicValueData::Value { id };
        let value_1 = SymbolicValue::new(0, data.clone());
    ...
    fn strict_equality_includes_instruction_pointer() {
        ...
        let data = SymbolicValueData::Value { id };
        let value_1 = SymbolicValue::new(0, data.clone());
        let value_2 = SymbolicValue::new(1, data);

        assert!(!value_1.strict_eq(&value_2));
    }

    #[test]
    fn strict_equality_includes_synthetic_flag() {
        let id = Uuid::new_v4();
        let data = SymbolicValueData::Value { id };
        let value_1 = SymbolicValue::new(0, data.clone());
